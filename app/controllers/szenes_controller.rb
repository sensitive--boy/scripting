class SzenesController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @szene.save
        format.html { redirect_to(script_videos_path(:id => @szene.sequence.video.id), :notice => 'Szene was successfully created.') }
  ...
  def update
    ...
    respond_to do |format|
      if @szene.update_attributes(params[:szene])
        format.html { redirect_to(script_videos_path(:id => @szene.sequence.video.id), :notice => 'Szene was successfully updated.') }
        format.xml  { head :ok }
      else
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to(script_videos_path(:id => @szene.sequence.video.id)) }
      format.xml  { head :ok }
    end
  end
